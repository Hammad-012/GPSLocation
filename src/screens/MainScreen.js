...
export default function MainScreen() {
  ...
  const _renderIcon = (routeName, selectedTab) => {
    ...
    switch (routeName) {
        case 'Home':
            icon = routeName === selectedTab ? <SelectedHomeIcon width={25} height={25} /> : <HomeIcon width={25} height={25} />;
            text = 'Home';
            break;
        case 'speedo-meter':
            icon = routeName === selectedTab ?  <SelectedSpeedoMeterIcon width={25} height={25} /> : <SpeedoMeterIcon width={25} height={25}  />;
            text = 'Speedo';
            break;
        case 'Search Number':
            icon = routeName === selectedTab ? < SelectedSearchIcon width={25} height={25}/> : <SearchIcon  width={25} height={25}/>
            text='Search'
            break;
        case 'Gps Tool':
            icon = routeName === selectedTab ? <SelectedGPStoolIcon width={25} height={25}/> : <GpstoolIcon width={25} height={25} />
            text='GPS Tool'
            break;
    }
    ...
    return (
      <View style={styles.tabItemContainer}>
        {icon}
        <Text style={{...styles.tabItemText, color: routeName === selectedTab ? 'blue' : 'gray'}}>{text}</Text>
      </View>
    );
  };

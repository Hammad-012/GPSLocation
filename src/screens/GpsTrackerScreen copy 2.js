...
const GpsTrackerScreen = () => {
  ...
  const [searchGeoCodingByLatLong, data] =
  ...
  useEffect(() => {
    if (all_data_saved && track_record.name !== '') {
      // console.log('saving',saving_track);
  ...
  const LocationfromCoords = (lat, lng, is_start) => {
    ...
    return new Promise((resolve, reject) => {
      fetch(
        ...
        .then(data => {
          console.log(data, 'NOMINATIM');
          if (is_start) {
            setTrackRecord({...track_record, start_address: data.display_name});
          } else {
            setTrackRecord({...track_record, end_address: data.display_name});
          }
          resolve();
        })
        .catch(error => {
